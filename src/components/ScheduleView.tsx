...
function DayCalendar(props : SubCalendarProps) {
  ...
  return (<>
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 overflow-scroll">
      {monthEvents.map((event) => (
        <div
          key={event.id}
          className={`cursor-pointer px-4 py-3.5 border border-gdg${event.calendar.color} border-l-6 hover:border-l-8 rounded-l-xs rounded-r-lg text-left hover:bg-neutral-200 dark:hover:bg-neutral-800`}
          onClick={() => setSelectedEvent(event)}
          >
          <h2 className={`font-bold truncate text-sm text-gdg${event.calendar.color}`}>{event.summary}</h2>
          <div className="text-xs text-neutral-700 dark:text-neutral-300 mt-1 mb-4 flex gap-2 items-center">
            {!event.isAllDay ? (
              <>
          ...
          <span className={`text-2xs text-white dark:text-neutral-900 rounded-full mb-2 px-3 py-1 bg-gdg${event.calendar.color}`}>{event.organizer.displayName || "Meetings"}</span>
        </div>
      ))}
    </div>
